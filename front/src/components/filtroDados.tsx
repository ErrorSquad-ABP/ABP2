...
const MultiSelect = ({ label, values, options, onChange }: any) => (
    <div>
        <Label>{label}</Label>
        <select multiple value={values} onChange={(e) => onChange(Array.from(e.target.selectedOptions, (o) => o.value))}>
            {options.map((opt: any) => (
                <option key={opt.value} value={opt.value}>
                    {opt.label}
                </option>
...
export const FiltroDados: React.FC<FiltroDadosProps> = ({ table, initial, onApply, onReset }) => {
    ...
    return (
        <Wrapper>
            ...
            <Row>
                <button onClick={handleApply} disabled={!responsibleType || !responsibleId || !start || !end}>
                    Gerar Gráfico
                </button>
